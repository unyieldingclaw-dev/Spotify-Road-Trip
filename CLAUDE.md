...
**Spotify Road Trip** is a desktop playlist expander. Keep these principles front-and-center:

### Core Requirement
**Zero duplicate URIs** — this is the feature. Every function must enforce deduplication at every step.

### Technology Stack (Windows 11 Only)
- **Python 3.10+** — no backend server, no Docker, no database
- **spotipy** — all Spotify API calls via this library
- **customtkinter** — dark-theme GUI (Windows primary)
- **pytest** — mock all Spotify calls in tests (no real API calls during testing)

### Authentication & Credentials
- **Authorization Code flow** — token cached locally in `.spotify_cache` (gitignored)
- **Never commit `.env`** — contains `SPOTIFY_CLIENT_ID` and `SPOTIFY_CLIENT_SECRET`

### GUI Invariants
- **No terminal window** — uses `pythonw.exe` + `run.bat`
- **Real-time progress** — user sees generation happening
- **Close-while-generating confirmation** — warns about partial playlists

### Design Decisions (Not Negotiable)
- Mock all Spotify in tests — respect API quota, reliable test runs
- URI deduplication (not track ID) — URIs are Spotify's canonical identifier
- Sliders for artist count and recommendation limit — user controls expansion size
